...
step3 -> step4 -> step5                          (comparison -> segmentation -> features)
                    \-> step7 -> step8           (leading/trailing + camera -> crossing parameters)
         step5 -> step6 -> step7/step8           (camera: sync+view -> segment -> features/params)
...
`Data/Camera CV/Test N/`), then [[Step 5 - Camera Sync and Viewers]] (needs `AllData` from step 3) →
[[Step 6 - Camera Stride Segmentation]] (needs `SegmentedParams` from step 4) →
[[Step 7 - Leading-Trailing Features (Camera)]] (needs FeatureLogs+Logger, `CameraSynced`) →
[[Step 8 - Crossing Parameters]] (needs `CameraSynced`, `SegmentedParams`, step-7 `S5`; prompts for leg length).
...
| [[Step 5 - Camera Sync and Viewers]] | sync camera markers to the 60 Hz IMU grid; angles/traj/3D viewers |
| [[Step 6 - Camera Stride Segmentation]] | cut camera foot markers into strides on the IMU ZVPs |
| [[Step 7 - Leading-Trailing Features (Camera)]] | step-5 features + camera, by side & leading/trailing |
| [[Step 8 - Crossing Parameters]] | foot placement + height/min clearance per crossing |
...
## Camera integration ✅
The camera is fully wired in as **steps 5–9** — see [[Camera Pipeline - Achievements]]. Refine the CV
